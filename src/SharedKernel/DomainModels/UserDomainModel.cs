...
public sealed class UserDomainModel
{
    ...
    /// <summary>Refresh token lifetime when the caller does not supply one.</summary>
    ...
    /// <summary>Lifetime of an e-mail confirmation token.</summary>
    ...
    public static ApplicationUser Create(CreateUserDTO createUserDTO, string passwordHash)
    {
        ...
        user.ApplicationUserTokens.Add(new ApplicationUserToken
        {
            ...
            ExpiryDate = DateTimeOffset.UtcNow.Add(EmailConfirmationTokenLifetime),
        });
    ...
    private string IssueToken(UserTokenType type, TimeSpan lifetime)
    {
        var rawToken = TokenGenerator.GenerateToken();
    ...
    private bool ConsumeToken(UserTokenType type, string? rawToken)
    {
        var now = DateTimeOffset.UtcNow;
